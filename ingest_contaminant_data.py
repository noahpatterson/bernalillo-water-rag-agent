...
def parse_numeric_value(value: str | None, numeric_type: type[NumericType]) -> NumericType | None:
  if value is None:
    return None
  if value == "":
    return None
...
def parse_TT_values_in_numeric_column(value: str | None, numeric_type: type[NumericType]) -> None:
  ...
  with open(path, 'r') as file:
    ...
    for row in reader:
      ...
      yield ComplianceResult(
        ...
        units=row["units"],
        lower_detection_limit=parse_numeric_value(row["lower_detection_limit"], float),
        upper_detection_limit=parse_TT_values_in_numeric_column(row["upper_detection_limit"], float),
        min_detected=parse_TT_values_in_numeric_column(row["min_detected"], float),
        ...
        max_detected=parse_TT_values_in_numeric_column(row["max_detected"], float),
        ...
        num_samples_exceeding_action_level=parse_numeric_value(row["num_samples_exceeding_action_level"], int),
        ninetieth_percentile=parse_numeric_value(row["ninetieth_percentile"], float),
        action_level=parse_numeric_value(row["Action Level"], float),
        uses_treatment_technique=row_uses_treatment_technique(row),
      )

def csv_to_db(connection: psycopg.Connection, csv_path: str, table_name: str, columns: list[str]):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.executemany(
                insert_query,
                [tuple(getattr(data, column) for column in columns) for data in parsed_data],
            )
        connection.commit()
...
def main():
    ...
    csv_to_db(connection, "data/processed/abcwua/CCR_Compliance_Results.csv", "compliance_results", columns)
    connection.close()
